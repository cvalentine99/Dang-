...
export default function Compliance() {
  ...
  const policies = useMemo(() => {
    if (!scaQ.data) return [];
    return extractItems(scaQ.data);
  }, [scaQ.data]);

  // ── Checks (real or fallback) ─────────────────────────────────────────
  const checks = useMemo(() => {
    if (!checksQ.data) return [];
    return extractItems(checksQ.data);
  }, [checksQ.data]);
  ...
  // ── Indexer compliance data ────────────────────────────────────────────
  const complianceSource: "indexer" | "mock" = indexerHealthy && complianceQ.data ? "indexer" : "mock";
  const complianceData = useMemo(() => {
    if (!complianceQ.data) return { total: 0, byControl: [] as { control: string; count: number }[], bySeverity: [] as { level: string; count: number }[], timeline: [] as { time: string; count: number }[] };
    const raw = complianceQ.data as Record<string, unknown>;
    const aggs = raw.aggregations as Record<string, unknown> | undefined;
    if (!aggs) return { total: 0, byControl: [] as { control: string; count: number }[], bySeverity: [] as { level: string; count: number }[], timeline: [] as { time: string; count: number }[] };
